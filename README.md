# 👩‍💻 Mariana Soares

🎓 Formada em Gestão da Tecnologia da Informação (CESAR School)  
💻 Estudante de Análise e Desenvolvimento de Sistemas (UNINTER)  
📚 Especialização em Gestão Ágil de Projetos  
🌱 Apaixonada por tecnologia com propósito e por pensar fora da caixa  
📍 Jaboatão dos Guararapes - PE

---

### 🚀 Sobre mim


✨ Olá! Sou Mariana, graduada em Gestão da Tecnologia da Informação pela CESAR School, atualmente cursando Análise e Desenvolvimento de Sistemas na UNINTER. 
   Minha trajetória começou na Publicidade, que despertou em mim a paixão por soluções criativas, mas foi na tecnologia que encontrei espaço para transformá-las em algo prático, inovador e escalável.

🎓 Atualmente, atuo como Tutora de Tecnologia no projeto **Florescendo Talentos**, onde ensino lógica de programação, boas práticas e desenvolvimento web para alunos do ensino médio público em Pernambuco. 
   Já participei de hackathons, liderei projetos com base em metodologias ágeis e desenvolvi soluções voltadas à análise de dados com foco em vulnerabilidade social e informalidade no varejo de Pernambuco.

🤝 Tenho interesse especial por **Scrum**, **tecnologias web**, **projetos colaborativos** e tudo que envolve **desenvolvimento com propósito**.


---

### 🛠️ Tecnologias & Ferramentas

- Linguagens: `HTML` · `CSS` · `JavaScript` · `Python`  
- Ferramentas: `Trello` · `Figma` · `Canva` . `Miro`   
- Metodologias: `Scrum` · `Kanban` · `PBL`  
- Em aprendizado: `Git` · `React` · `Node.js`

---

### 📌 Projetos em destaque


- 👩‍🏫 **Florescendo Talentos**  
  Atividades educacionais com foco em lógica, Python e desenvolvimento web.


---

### 🌐 Onde me encontrar

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/marianaosoares/)  
📫 marioliveirasoares@gmail.com

---

### ✨ Curiosidades

- 🎯 Meu propósito é entregar valor através da tecnologia, com empatia e visão social.  
- 🤝 Valorizo ambientes colaborativos, inclusivos e com espaço para crescer.  
- 🚀 Estou aberta a projetos, parcerias e conexões com propósito!

